...
export class AudioStreamer {
  ...
  constructor(public context: AudioContext, sampleRate?: number) {
    this.sampleRate = sampleRate || context.sampleRate || 24000;
  ...
  addPCM16(chunk: Uint8Array) {
    console.log('[AudioStreamer] Processing PCM16 chunk:', chunk.length, 'bytes, sample rate:', this.sampleRate);
    ...
    console.log('[AudioStreamer] Processing buffer size:', this.processingBuffer.length, 'samples');

    while (this.processingBuffer.length >= this.bufferSize) {
      const buffer = this.processingBuffer.slice(0, this.bufferSize);
      this.audioQueue.push(buffer);
      this.processingBuffer = this.processingBuffer.slice(this.bufferSize);
      console.log('[AudioStreamer] Added buffer to queue, queue length:', this.audioQueue.length);
    }

    if (!this.isPlaying) {
      console.log('[AudioStreamer] Starting audio playback');
      this.isPlaying = true;
